...
class BeamMessageBrokerTest {
  ...
  void send() {
    ...
    var task = BeamTask.of(MESSAGE_ID, APP_ID, List.of(DESTINATION), body);
  ...
  void receive_oneTaskWithInvalidBody(String decodedBody) {
    var task = BeamTask.of(TASK_ID, APP_ID, List.of(APP_ID), base64Encode(decodedBody));
  ...
  void receive_oneTaskWithInvalidBodyAndClaimError() {
    var task = BeamTask.of(TASK_ID, APP_ID, List.of(APP_ID), base64Encode("{}"));
    when(client.retrieveTasks()).thenReturn(Flux.just(task));
  ...
  void receive_oneNonMatchingTask() {
    var task = BeamTask.of(TASK_ID, APP_ID, List.of(APP_ID), base64Encode(VALID_TASK_BODY));
    when(client.retrieveTasks()).thenReturn(Flux.just(task));
    when(client.claimTask(task)).thenReturn(Mono.empty());
  ...
  void parseBody_completeOnInvalid(String body, String errorMessagePrefix) {
    var task = BeamTask.of(TASK_ID, APP_ID, List.of(APP_ID), body);
